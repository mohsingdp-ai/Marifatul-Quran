...
for (const key of Object.keys(T)) {
  ...
  for (let i = 0; i < t.ayahs.length; i++) {
    ...
    checks++;
    const got = recitingAyahAt(t, probe);
  ...
  for (let i = 0; i < t.ayahs.length - 1; i++) {
    checks++;
    const got = recitingAyahAt(t, t.ayahs[i + 1][1] - 0.01);
    if (got !== t.ayahs[i][0]) fails.push(`${key} ayah ${t.ayahs[i][0]} lost early -> ${got}`);
  }
  // 3. nothing is lit before the first placed ayah. This branch does not use trim, so the
  //    intro plays as it always did and the panel stays dark until the first ayah arrives.
  const first = t.ayahs[0][1];
  if (first > 1) {
    checks++;
    const got = recitingAyahAt(t, first - 0.5);
    if (got !== null) fails.push(`${key} lit ${got} before its first placed ayah`);
  }
  // 4. every number it can emit exists in the panel
